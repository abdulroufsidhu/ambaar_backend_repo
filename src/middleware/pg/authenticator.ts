...
import { Employee, User } from "../../models/pg";
...
export const Authenticator = {
  requireUser: (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      const decoded = jwt.verify(token, config.JWT.key) as User; // Verify the token Replace 'your-secret-key' with your actual secret key
  ...
  requireSelf: (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const resourceId = !!req.params.uid ? req.params.uid : req.query.uid ;
