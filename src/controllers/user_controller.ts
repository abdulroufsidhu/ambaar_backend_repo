...
class UserController {
  create = async (user: IUser) => {
    ...
    return personController.create(user.person).then((person) =>
      this.fromPerson(person._id)
        .then((user) => user)
        .catch(() => {
          return new User({ person, password: user.password })
            .save()
            .then((user) => {
              return this.fromId(user._id);
            });
        })
    );
  };

  fromPerson = async (person_id: string) =>
    User.findOne({ person: person_id }).then((user) => {
      if (!!user) return user;
      throw new Error("User not found");
    });
  fromEmail = async (email: string, password: string) =>
  ...
  fromId = async (id: string) => User.findById(id).then((user) => user);

  remove = async (id: string) =>
    User.findByIdAndDelete(id).then((user) => user);

  fromToken = async (token: string) =>
  ...
  readReq = async (req: Request, res: Response, next: NextFunction) => {
    ...
    return this.fromEmail(email, password)
      .then((user) => {
        if (user) {
          const token = jwt.sign(
            { _id: user._id, person: user.person },
