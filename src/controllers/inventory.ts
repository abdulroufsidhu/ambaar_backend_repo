...
const fromProduct = async (product_id: string, branch_id: string) => Inventory.find({ product: product_id, branch: branch_id }).populate(["product", "branch"]).then(inv => inv);
const fromBranch = async (branch_id: string) => Inventory.find({ branch: branch_id }).populate(["product", "branch"]).then(inv => inv);
...
const readReq = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (typeof id === "string") {
    return fromId(id)
      .then(inventory => inventory ? res.status(200).json({ inventory }) : res.status(404).json({ message: "no data found" }))
      .catch(error => res.status(500).json({ error }));
  }
  if (typeof product_id === "string") {
    if (typeof branch_id === "string") {
      ...
      return res.status(500).json({ message: "Be sure to provide branch_id if you provide product_id" });
    }
  }
  if (typeof branch_id === "string") {
  ...
  return res.status(500).json({ message: "Be sure to provide id, product_id or branch_id" })
}
